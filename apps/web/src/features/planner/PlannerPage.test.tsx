...
const BADKAMER = makeTask({ _id: 't1', name: 'Badkamer', roomId: 'r1', intervalKey: '1w', durationMinutes: 40 });
const STOFZUIGEN = makeTask({ _id: 't2', name: 'Stofzuigen', roomId: 'r1', intervalKey: '1w', durationMinutes: 30 });
const RAMEN = makeTask({ _id: 't3', name: 'Ramen', roomId: 'r1', intervalKey: 'quarter', durationMinutes: 90 });
...
describe('PlannerPage — drops', () => {
  ...
  it('places an accepted drop and saves it after the debounce', async () => {
    const fetchMock = setup([makePlan({ _id: 'p1', name: 'Standaard', active: true })], {
      'PUT /api/cycle-plans/p1/slots': (init: RequestInit) => ({
        plan: makePlan({ _id: 'p1', name: 'Standaard', active: true, slots: JSON.parse(String(init.body)).slots }),
        warnings: [],
  ...
  it('removes a slot via its button as an alternative to dragging', async () => {
    setup([makePlan({ _id: 'p1', name: 'Standaard', active: true, slots: [slot('t2', 1, 3, null)] })], {
      'PUT /api/cycle-plans/p1/slots': { plan: makePlan({ _id: 'p1', name: 'Standaard' }), warnings: [], summary: {} },
    });
    renderWithProviders(<PlannerPage />);
...
describe('PlannerPage — budgets and pool', () => {
  ...
  it('automatically collapses the task pool when every distributable task is planned', async () => {
    ...
    const pool = await screen.findByRole('complementary', { name: 'Nog in te plannen' });
    await waitFor(() =>
      expect(within(pool).getByRole('button', { name: 'Nog in te plannen uitklappen' })).toBeInTheDocument(),
    );
    expect(within(pool).queryByLabelText('Filter taken op ruimte')).not.toBeInTheDocument();
  });
});
...
describe('PlannerPage — activation', () => {
  ...
  it('asks for confirmation with the replacement rule before activating', async () => {
    const fetchMock = setup(
      [makePlan({ _id: 'p1', name: 'Standaard', active: true }), makePlan({ _id: 'p2', name: 'Zomer' })],
      { 'POST /api/cycle-plans/p2/activate': { plan: makePlan({ _id: 'p2', name: 'Zomer', active: true }), removed: 3 } },
    );
    renderWithProviders(<PlannerPage />);
    await openPlanManagement();
    ...
    expect(dialog).toHaveTextContent('Afgevinkte, overgeslagen, verplaatste en losse taken blijven staan.');
    fireEvent.click(within(dialog).getByRole('button', { name: 'Activeren' }));

    await waitFor(() =>
      expect(fetchMock.mock.calls.some(([url]) => url === '/api/cycle-plans/p2/activate')).toBe(true),
    );
