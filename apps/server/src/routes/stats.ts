...
export const statsRoutes: FastifyPluginAsync = async (app) => {
  app.get('/stats/workload', async (request) => {
    ...
    return workloadStats(app.deps.db, app.deps.clock.now(), cycles);
  });

  app.get('/stats/completion', async (request) => {
    const { cycles, groupBy } = parseOrThrow(completionQuerySchema, request.query);
    return completionStats(app.deps.db, app.deps.clock.now(), cycles, groupBy);
  });
